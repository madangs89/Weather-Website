...
// Async thunks
export const loadCityCurrent = createAsyncThunk(
  "weather/loadCurrent",
  async (city) => {
...
export const loadCityForecast = createAsyncThunk(
...
const weatherSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      .addCase(loadCityCurrent.pending, (s) => {
        s.status = "loading";
      })
      .addCase(loadCityCurrent.fulfilled, (s, a) => {
        ...
        s.cities[city] = {
          ...(s.cities[city] || {}),
          current: a.payload.data,
          lastUpdated: Date.now(),
        };
        s.status = "succeeded";
      })
      .addCase(loadCityCurrent.rejected, (s, a) => {
        s.status = "failed";
        s.error = a.error.message;
      })
      .addCase(loadCityForecast.pending, (s) => {
        s.status = "loading";
      })
      .addCase(loadCityForecast.fulfilled, (s, a) => {
        ...
        s.cities[city] = {
          ...(s.cities[city] || {}),
          forecast: a.payload.data,
          lastUpdated: Date.now(),
        };
        s.status = "succeeded";
      })
      .addCase(loadCityForecast.rejected, (s, a) => {
        s.status = "failed";
        s.error = a.error.message;
      });
  },
...
export const { addFavorite, removeFavorite, setUnit, setCityData , setRefreshInterval } =
